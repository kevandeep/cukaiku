...
export function ReliefBreakdown({ r }: { r: ComputeResult }) {
  ...
  return (
    <div className="mb-7">
      <h3 className="text-sm font-semibold text-slate-100 mb-3">{t('reliefBreakdown')}</h3>
      <div className="flex flex-col gap-1">
        {sorted.map(rl => (
          <div key={rl.ref} className="bg-slate-900 rounded-lg px-3.5 py-2.5 border border-slate-800">
            <div className="flex justify-between items-center mb-1.5">
              <div>
                <span className="text-[10px] text-slate-600 font-mono mr-1.5">{rl.ref}</span>
                <span className="text-xs text-slate-300">{rl.name}</span>
              </div>
