...
export function FormBETemplate({ r }: { r: ComputeResult }) {
  ...
  return (
    <div className="mb-8">
      <h3 className="text-base font-bold text-slate-100 mb-1">{t('formTitle')}</h3>
      <p className="text-xs text-slate-500 mb-5 leading-relaxed">{t('formSubtitle')}</p>

      <div className="bg-slate-900 rounded-2xl border border-slate-800 overflow-hidden">
        {/* Form header */}
        <div className="flex justify-between items-center px-5 py-4 border-b border-slate-800 bg-gradient-to-r from-slate-800/50 to-slate-900">
          <div>
            <div className="text-[9px] font-mono text-slate-600 tracking-widest uppercase mb-1">
              Lembaga Hasil Dalam Negeri Malaysia
            </div>
            <div className="text-lg font-bold text-slate-100">FORM BE 2025</div>
            <div className="text-xs text-slate-500">Resident Individual — Employment Income</div>
          </div>
          <div className="text-right">
            <div className="text-[9px] font-mono text-slate-600 uppercase">Year of Assessment</div>
        ...
        <div className="px-5 py-3 border-b border-slate-800">
          <div className="flex items-center gap-2 mb-1.5">
            ...
            <span className="text-xs font-bold text-slate-400">{t('sectionA')}</span>
          </div>
          <p className="text-xs text-slate-600">{t('sectionANote')}</p>
        </div>
        ...
        {SECTIONS.map(sec => {
          ...
          return (
            <div key={sec.key} className="border-b border-slate-800 last:border-0">
              ...
              {fields.map((f, i) => (
                <div
                  key={i}
                  className="flex justify-between items-center px-5 py-2 border-t border-slate-800/50"
                  style={{
                ...
                >
                  <div className="flex-1 min-w-0 mr-4">
                    <span className="text-[10px] font-mono text-slate-600 mr-2">{f.ref}</span>
                    <span className={`text-xs ${f.bold ? 'text-slate-100 font-bold' : 'text-slate-400'}`}>
                      {f.label}
                    </span>
                  </div>
                  <div
                    ...
                    style={{
                      color: f.value > 0 ? (f.bold ? sec.color : '#e2e8f0') : '#334155',
      ...
      <p className="text-xs text-slate-600 text-center mt-3 leading-relaxed">{t('formBENote')}</p>
    </div>
  );
}
